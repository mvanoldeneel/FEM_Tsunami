...
void femTsunamiAddIntegralsElement(femTsunami *myTsunami){
	...
	for(iElem=0;iElem<myTsunami->mesh->nElem;++iElem){
		...
		for(k=0;k<theRule->n;++k){
			...
			double lat = ((4*R*R-x*x-y*y))/(4*R*R+x*x+y*y);
...
void femTsunamiAddIntegralsEdges(femTsunami *myTsunami){
	...
	for(iEdge=0;iEdge<myTsunami->edges->nEdge;++iEdge){
		...
		for(k=0;k<2;++k){
			...
			machin = ((4*R*R+x*x+y*y)/(4*R*R));
			
			for(i=0;i<2;++i){
				BE[mapEdge[0][i]] -= phiEdge[i]*h*unstar*machin   * jac*weight;
				BU[mapEdge[0][i]] -= phiEdge[i]*nx*g*estar*machin * jac*weight;
				BV[mapEdge[0][i]] -= phiEdge[i]*ny*g*estar*machin * jac*weight;
				BE[mapEdge[1][i]] += phiEdge[i]*h*unstar*machin   * jac*weight;
				BU[mapEdge[1][i]] += phiEdge[i]*nx*g*estar*machin * jac*weight;
				BV[mapEdge[1][i]] += phiEdge[i]*ny*g*estar*machin * jac*weight;
			}
		}
	}	
}
